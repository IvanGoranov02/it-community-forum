...
export default async function DebugAddIsArchivedPage() {
  try {
    const supabase = createServerClient()

    // Execute SQL to add is_archived column
    const { data, error } = await supabase.rpc("execute_sql", {
      sql_query: "ALTER TABLE posts ADD COLUMN IF NOT EXISTS is_archived BOOLEAN DEFAULT FALSE",
    })

    // Check if column exists
    const { data: columnCheck, error: columnError } = await supabase.rpc("execute_sql", {
      sql_query:
    ...
    return (
      <div className="container mx-auto px-4 py-6">
        <h1 className="text-3xl font-bold mb-6">Debug Add is_archived Column</h1>
    ...
    console.error("Unexpected error in DebugAddIsArchivedPage:", error)
    return (
      <div className="container mx-auto px-4 py-6">
